...
lt = [j for j in range(1, end + 1) if sum([(ord(i) - 48)
                                           ** len(str(j)) for i in str(j)]) == j]
print(lt)
print('-' * 50)
    ...
    def search_num(cur_index, sum, remain_num):
        ...
        if cur_index == 0:
            ...
            for i in range(remain_num + 1):
                ...
                search_num(cur_index - 1, sum +
                           pre_table1[cur_index][i], remain_num - i)
